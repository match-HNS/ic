...
public plugin_init() {
	register_plugin(PLUGIN, "4.0.4", AUTHOR);
...
public message__say_text(msgid, dest, receiver) {
	...
	// ★ Use normal chat channel and put the formatted message in arg4 so it appears in the bottom-left chat, not center screen
	set_msg_arg_string(3, "");
	set_msg_arg_string(4, g_sMessage);
